...
def make_celery() -> Celery:
    ...
    automation_poll_interval = _env_int("AUTOMATION_POLL_INTERVAL", 60, minimum=15)
    automation_dispatch_interval = _env_int("AUTOMATION_DISPATCH_INTERVAL", 30, minimum=10)
    ...
    celery_app.conf.beat_schedule = {
        ...
        "automation-poll-events": {
            "task": "automation.poll_events",
            "schedule": automation_poll_interval,
        },
        "automation-dispatch-events": {
            "task": "automation.dispatch_events",
            "schedule": automation_dispatch_interval,
        },
        "automation-dispatch-actions": {
            "task": "automation.dispatch_actions",
            "schedule": automation_dispatch_interval,
        },
        "automation-dispatch-notifications": {
...
_AUTOMATION_REDIS_CLIENT: object | None = None
_AUTOMATION_REDIS_LOCK = Lock()


def _automation_redis_client():
    global _AUTOMATION_REDIS_CLIENT
    if _AUTOMATION_REDIS_CLIENT is not None:
        return _AUTOMATION_REDIS_CLIENT
    with _AUTOMATION_REDIS_LOCK:
        if _AUTOMATION_REDIS_CLIENT is not None:
            return _AUTOMATION_REDIS_CLIENT
        try:
        ...
        try:
            client = redis.Redis.from_url(url, decode_responses=True, **socket_options)
            client.ping()
        except redis.exceptions.RedisError:  # pragma: no cover - runtime path
            return None
        _AUTOMATION_REDIS_CLIENT = client
        return _AUTOMATION_REDIS_CLIENT


def _automation_queue_name(kind: str) -> str:
    return f"automation:{kind}"


def _automation_push_queue(kind: str, item_id: int) -> None:
    client = _automation_redis_client()
    if client is None:
        return
    try:
        client.rpush(_automation_queue_name(kind), str(item_id))
    except RedisError:  # pragma: no cover - runtime path
        return


def _automation_pop_queue(kind: str, limit: int = 50) -> List[int]:
    client = _automation_redis_client()
    if client is None:
        return []
    items: List[int] = []
    try:
        for _ in range(limit):
            raw = client.lpop(_automation_queue_name(kind))
            if raw is None:
                break
            try:
...
def _build_filter_clause(filters: List[dict]) -> Tuple[str, List[object]]:
    clauses: List[str] = []
    params: List[object] = []
    allowed_ops = {"=", "!=", ">", ">=", "<", "<=", "in", "not in", "like"}
    for raw in filters:
        column = _safe_identifier(str(raw.get("column", "")).strip())
...
def _automation_poll_event_type(event_type: dict, *, limit: int = 200) -> int:
    source_query = (event_type.get("source_query") or "").strip()
    source_table = _safe_identifier(str(event_type.get("source_table") or "").strip())
    if not source_query:
        return 0
    cursor_type = event_type.get("cursor_type") or "id"
    cursor_value = event_type.get("cursor_value")
    query = source_query.replace("{{cursor}}", "%s").replace("{{limit}}", "%s")
    params: List[object] = []
    if "{{cursor}}" in source_query:
        params.append(cursor_value)
    if "{{limit}}" in source_query:
        params.append(limit)
    ...
    for row in rows:
        ...
        if user_id is None:
            continue
        event_cursor = row.get("event_cursor") or row.get("cursor") or row.get("id")
        payload = {
        ...
        event_id = db.create_event_inbox_entry(
            {
                "event_type_id": event_type.get("id"),
                "user_id": int(user_id),
                "source_table": source_table,
                "source_pk": str(event_cursor) if event_cursor is not None else None,
                "source": "poller",
                "payload": payload,
                "status": "pending",
            }
        )
        _automation_push_queue("events", event_id)
        current_cursor = event_cursor
        if current_cursor is not None:
            max_cursor_value = current_cursor

    if max_cursor_value is not None:
        db.update_automation_event_cursor(int(event_type["id"]), str(max_cursor_value))
    return len(rows)


def _resolve_context_user(user_id: int) -> dict | None:
    try:
...
def _select_language(user: dict | None, payload: dict) -> str:
    if isinstance(payload.get("language"), str):
        return payload.get("language").lower()
    if user and isinstance(user.get("pk"), str) and user.get("pk"):
        return user.get("pk").lower()
    return "en"


def _render_template_by_language(template_data: dict, language: str) -> str:
    if not template_data:
        return ""
    if language in template_data:
        return template_data.get(language) or ""
    if language[:2] in template_data:
        return template_data.get(language[:2]) or ""
    if "en" in template_data:
        return template_data.get("en") or ""
...
def _render_message_template(message: dict, user: dict | None, payload: dict) -> Tuple[str, str]:
    language = _select_language(user, payload)
    subject_tpl = _render_template_by_language(message.get("subject") or {}, language)
    body_tpl = _render_template_by_language(message.get("body") or {}, language)
    context = {
        "user": user or {},
        "payload": payload,
    }
    subject = Template(subject_tpl).render(**context) if subject_tpl else ""
    body = Template(body_tpl).render(**context) if body_tpl else ""
    return subject, body


def _parse_json_input(value, default):
    if value is None:
        return default
...
def _send_email_message(to_address: str, subject: str, body: str) -> None:
    ...
    smtp_from = os.getenv("SMTP_FROM") or smtp_user or "no-reply@examboot.net"
...
def _evaluate_condition(condition: dict, context: dict) -> bool:
    if not condition:
        return True
    if "all" in condition:
        return all(_evaluate_condition(item, context) for item in condition.get("all", []))
    if "any" in condition:
        return any(_evaluate_condition(item, context) for item in condition.get("any", []))

    field = condition.get("field")
    operator = (condition.get("operator") or "eq").lower()
    ...
    if operator == "contains":
        return isinstance(value, (list, str)) and target_value in value
    if operator == "in":
        return isinstance(target_value, list) and value in target_value
    return False


def _execute_action(action: dict, user: dict | None, payload: dict) -> None:
    action_type = action.get("action_type")
    config = action.get("config") or {}
    field = _safe_identifier(str(config.get("field") or ""))
    if action_type == "update_user_field":
        if not field:
            raise ValueError("Champ user manquant.")
        conn = db.get_connection()
        cursor = conn.cursor()
        cursor.execute(f"UPDATE users SET `{field}` = %s WHERE id = %s", (config.get("value"), user["id"]))
        conn.commit()
        cursor.close()
        conn.close()
        return
    if action_type == "increment_user_field":
        if not field:
            raise ValueError("Champ user manquant.")
    ...
    if action_type == "webhook":
        ...
        body = Template(body_template).render(user=user or {}, payload=payload) if body_template else None
        requests.request(method, url, headers=headers, data=body, timeout=20)
        return
    raise ValueError(f"Action non supportée: {action_type}")


def _process_event_rules(event: dict) -> None:
    ...
    try:
        ...
        rules = []
    matching_rules = [rule for rule in rules if int(rule.get("event_type_id") or 0) == event_type_id]
    user = _resolve_context_user(user_id)
    context = {"payload": payload, "user": user or {}}

    for rule in matching_rules:
        if db.rule_execution_exists(rule["id"], event_id):
            continue
        try:
            ...
            if rule.get("action_id"):
                action_queue_id = db.enqueue_action(
                    {
                ...
                _automation_push_queue("actions", action_queue_id)

            if rule.get("message_id") and rule.get("channels"):
                ...
                _automation_push_queue("notifications", notification_id)
            if action_queue_id or notification_id:
                db.insert_rule_execution(
                    rule["id"],
                    event_id,
                    action_queue_id=action_queue_id,
                    notification_queue_id=notification_id,
                )
        except Exception as exc:
            app.logger.exception("Automation rule failed: rule_id=%s event_id=%s", rule["id"], event_id)


@celery_app.task(name="automation.poll_events")
def automation_poll_events() -> Dict[str, object]:
    event_types = db.get_automation_event_types()
    totals = {"event_types": len(event_types), "events": 0}
    for event_type in event_types:
        if not event_type.get("enabled", True):
            continue
        totals["events"] += _automation_poll_event_type(event_type)
    return totals


@celery_app.task(bind=True, name="automation.process_event")
def automation_process_event(self, event_id: int) -> None:
    event = db.get_event_inbox_entry(event_id)
    if not event:
        return
    if event.get("status") == "processed":
        return
    db.update_event_inbox_status(event_id, "processing")
    try:
        _process_event_rules(event)
        db.update_event_inbox_status(event_id, "processed")
    except Exception as exc:
        db.update_event_inbox_status(event_id, "failed", error=str(exc))


def _dispatch_queue(kind: str, pending_fetch, task_handler, mark_status=None) -> int:
    dispatched = 0
    queued_ids = _automation_pop_queue(kind, limit=50)
    if not queued_ids:
        queued_ids = [entry["id"] for entry in pending_fetch(limit=50)]
    for item_id in queued_ids:
        if mark_status:
            try:
                mark_status(item_id, "queued")
            except Exception:
                pass
        try:
            task_handler.apply_async(args=(item_id,))
        except (CeleryError, OperationalError):
            task_handler(item_id)
        dispatched += 1
    return dispatched


@celery_app.task(name="automation.dispatch_events")
def automation_dispatch_events() -> Dict[str, int]:
    dispatched = _dispatch_queue(
        "events", db.get_pending_event_inbox, automation_process_event, db.update_event_inbox_status
    )
    return {"dispatched": dispatched}


@celery_app.task(bind=True, name="automation.execute_action")
def automation_execute_action(self, action_queue_id: int) -> None:
    entry = db.get_action_queue_entry(action_queue_id)
    if not entry:
        return
    if entry.get("status") == "processed":
        return
    db.update_action_queue_status(action_queue_id, "processing")
    action = db.get_automation_action(entry["action_id"])
    if not action:
        db.update_action_queue_status(action_queue_id, "failed", error="Action introuvable")
        return
    user = _resolve_context_user(int(entry["user_id"]))
    payload = entry.get("payload") or {}
    merged_action = dict(action)
    merged_action["config"] = {
        **(action.get("config") or {}),
        **(action.get("action_overrides") or {}),
        **(payload.get("action_override") or {}),
    }
    try:
        _execute_action(merged_action, user, payload)
        db.update_action_queue_status(action_queue_id, "processed")
    except Exception as exc:
        db.update_action_queue_status(action_queue_id, "failed", error=str(exc))


@celery_app.task(name="automation.dispatch_actions")
def automation_dispatch_actions() -> Dict[str, int]:
    dispatched = _dispatch_queue(
        "actions", db.get_pending_action_queue, automation_execute_action, db.update_action_queue_status
    )
    return {"dispatched": dispatched}


@celery_app.task(bind=True, name="automation.execute_notification")
def automation_execute_notification(self, notification_id: int) -> None:
    entry = db.get_notification_queue_entry(notification_id)
    if not entry:
        return
    if entry.get("status") == "processed":
        return
    db.update_notification_queue_status(notification_id, "processing")
    message = db.get_automation_message(entry["message_id"])
    if not message:
        db.update_notification_queue_status(notification_id, "failed", error="Message introuvable")
        return
    user = _resolve_context_user(int(entry["user_id"]))
    payload = entry.get("payload") or {}
    subject, body = _render_message_template(message, user, payload)
    channels = entry.get("channels") or []
    errors: List[str] = []
    for channel in channels:
        try:
            if channel == "email":
                ...
                _send_email_message(user["email"], subject or message.get("name") or "Notification", body)
            elif channel == "inbox":
                if not user:
                    raise ValueError("Utilisateur introuvable")
                db.create_in_app_message(int(user["id"]), subject or message.get("name") or "Notification", body)
            else:
                raise ValueError(f"Canal non supporté: {channel}")
        except Exception as exc:
            errors.append(f"{channel}: {exc}")
    if errors:
        db.update_notification_queue_status(notification_id, "failed", error="; ".join(errors))
    else:
        db.update_notification_queue_status(notification_id, "processed")


@celery_app.task(name="automation.dispatch_notifications")
def automation_dispatch_notifications() -> Dict[str, int]:
    dispatched = _dispatch_queue(
        "notifications",
        db.get_pending_notification_queue,
        automation_execute_notification,
        db.update_notification_queue_status,
    )
    return {"dispatched": dispatched}
...
def _normalise_enabled(value) -> bool:
    if isinstance(value, bool):
        return value
    if isinstance(value, str):
        return value.lower() in {"1", "true", "yes", "on"}
...
@app.route("/automation/api/events", methods=["GET", "POST"])
def automation_events():
    if request.method == "GET":
        return jsonify(db.get_automation_event_types())

    payload = request.get_json() or {}
    payload["filters"] = _parse_json_input(payload.get("filters"), [])
    payload["payload_columns"] = _parse_json_input(payload.get("payload_columns"), [])
    payload["enabled"] = _normalise_enabled(payload.get("enabled", True))
    payload["source_query"] = payload.get("source_query")
    event_id = db.create_automation_event_type(payload)
    return jsonify({"id": event_id})


@app.route("/automation/api/events/<int:event_id>", methods=["PUT", "DELETE"])
def automation_event_detail(event_id: int):
    if request.method == "DELETE":
        db.delete_automation_event_type(event_id)
    ...
    payload["enabled"] = _normalise_enabled(payload.get("enabled", True))
    payload["source_query"] = payload.get("source_query")
    db.update_automation_event_type(event_id, payload)
    return jsonify({"status": "updated"})


@app.route("/automation/api/actions", methods=["GET", "POST"])
def automation_actions():
    if request.method == "GET":
        return jsonify(db.get_automation_actions())

    payload = request.get_json() or {}
    payload["config"] = _parse_json_input(payload.get("config"), {})
    payload["action_overrides"] = _parse_json_input(payload.get("action_overrides"), {})
    payload["enabled"] = _normalise_enabled(payload.get("enabled", True))
    action_id = db.create_automation_action(payload)
    return jsonify({"id": action_id})


@app.route("/automation/api/actions/<int:action_id>", methods=["PUT", "DELETE"])
def automation_action_detail(action_id: int):
    if request.method == "DELETE":
        db.delete_automation_action(action_id)
        return jsonify({"status": "deleted"})
    ...
    payload["action_overrides"] = _parse_json_input(payload.get("action_overrides"), {})
    payload["enabled"] = _normalise_enabled(payload.get("enabled", True))
    db.update_automation_action(action_id, payload)
    return jsonify({"status": "updated"})


@app.route("/automation/api/messages", methods=["GET", "POST"])
def automation_messages():
    if request.method == "GET":
        return jsonify(db.get_automation_messages())

    payload = request.get_json() or {}
    payload["subject"] = _parse_json_input(payload.get("subject"), {})
    payload["body"] = _parse_json_input(payload.get("body"), {})
    payload["enabled"] = _normalise_enabled(payload.get("enabled", True))
    message_id = db.create_automation_message(payload)
    return jsonify({"id": message_id})


@app.route("/automation/api/messages/<int:message_id>", methods=["PUT", "DELETE"])
def automation_message_detail(message_id: int):
    if request.method == "DELETE":
        db.delete_automation_message(message_id)
        return jsonify({"status": "deleted"})
    ...
    payload["body"] = _parse_json_input(payload.get("body"), {})
    payload["enabled"] = _normalise_enabled(payload.get("enabled", True))
    db.update_automation_message(message_id, payload)
    return jsonify({"status": "updated"})


@app.route("/automation/api/rules", methods=["GET", "POST"])
...
@app.route("/automation/api/rules/<int:rule_id>", methods=["PUT", "DELETE"])
def automation_rule_detail(rule_id: int):
    if request.method == "DELETE":
...
@app.route("/automation/api/test-event", methods=["POST"])
def automation_test_event():
    payload = request.get_json() or {}
    event_type_id = payload.get("event_type_id")
    user_email = payload.get("user_email")
    event_payload = _parse_json_input(payload.get("payload"), {})
    if not event_type_id or not user_email:
        return jsonify({"error": "event_type_id et user_email requis"}), 400

    user = db.get_user_by_email(user_email)
    if not user:
        return jsonify({"error": "Utilisateur introuvable"}), 404

    event_type = db.get_automation_event_type(int(event_type_id))
    event_data = {
        "event_code": event_type.get("code") if event_type else None,
        "event_name": event_type.get("name") if event_type else None,
        "source": "tester",
        "data": event_payload,
    }
    event_id = db.create_event_inbox_entry(
    ...
    _automation_push_queue("events", event_id)
    return jsonify({"event_id": event_id, "status": "queued"})


@app.route("/automation/api/history/events")
def automation_history_events():
    status = request.args.get("status")
    limit = int(request.args.get("limit", "200"))
    return jsonify(db.get_automation_event_history(limit=limit, status=status))


@app.route("/automation/api/history/actions")
def automation_history_actions():
    status = request.args.get("status")
    limit = int(request.args.get("limit", "200"))
    return jsonify(db.get_automation_action_history(limit=limit, status=status))


@app.route("/automation/api/history/notifications")
def automation_history_notifications():
    status = request.args.get("status")
    limit = int(request.args.get("limit", "200"))
    return jsonify(db.get_automation_notification_history(limit=limit, status=status))


@app.route("/automation/api/history/rules")
def automation_history_rules():
    status = request.args.get("status")
    limit = int(request.args.get("limit", "200"))
    return jsonify(db.get_automation_rule_history(limit=limit, status=status))
